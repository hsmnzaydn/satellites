...
import com.hsmnzaydn.satellites.utils.fragment_controller.animation.Animation
import com.hsmnzaydn.satellites.utils.fragment_controller.custom.create
import com.hsmnzaydn.satellites.utils.fragment_controller.data.Controller
...
class FragmentController(private val activity: Activity, private val containerId: Int) : Navigator {
    ...
    private fun navigate(controllerName: String, fragmentOption: FragmentOption) {
        ...
        if (controller != null && fragmentManager != null) {
            ...
            if (currentFragment() != null && currentFragment()?.tag ?: "" == fragTag && !fragmentOption.reCreate) {
    ...
    override fun goBackFragment(tag: String) {
        try {
            if (fragmentStack.size > 1) {
                val currentFm =
                ...
                if (tag.isNotEmpty()) {
                    dest = fragmentStack.find { it.tag == tag }
                        ?: throw NullPointerException("Not Found Popup fragment")

                    val popupFragmentIndex = fragmentStack.indexOfFirst { it.tag == tag }
